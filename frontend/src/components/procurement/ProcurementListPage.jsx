...
export default function ProcurementListPage({ role, currentPath, hasWriteAccess = false, hasApprovalAccess = false }) {
  ...
  const handleSaveProcurement = () => {
    ...
    setView('list');
    setSuccessMessage('Surat pengajuan pengadaan aset berhasil dibuat dan disimpan.');
  ...
  const handleConfirmDecision = (e) => {
    ...
    const updatedProcurements = procurements.map(proc => {
      if (proc.id === procurementId) {
        ...
        // - if all items approved: status = approved
        // - if all items rejected: status = rejected
        // - else: pending
